...
int Alarm = 0;
int diag = 0;                    // abilita diagnostica su seriale
...
*/     
  ...
  if (diag == 1) {
    Serial.print("LastTempCamino:  ");    
    Serial.println(LastTempCamino); 
    Serial.print("TempCamino:  ");    
    Serial.println(TempCamino); 
  }
  if (ModSetpoint == 0){
...
///////////////  CONTROLLO QUALE PULSANTE E' STATO PREMUTO PREMUTO  ///////////////
      ...
      if (xpos > 250 && xpos < 300  && ypos > 90 && ypos < 130){  // PULSANTE SELEZIONE MODO RISCALDAMENTO
        StatoPulsSelRisc = 1;
        if (diag == 1) Serial.println(" click riscaldamento! ");
        }
      if (xpos > 250 && xpos < 300  && ypos > 150 && ypos < 190){ // PULSANTE SELEZIONE MODO ACQUA SANITARI
        StatoPulsSelSan = 1;
        if (diag == 1) Serial.println(" click acqua sanitari! ");
        }
      if (xpos > 60 && xpos < 180  && ypos > 50 && ypos < 90){    // PULSANTE SELEZIONE MODIFICA SETPOINT
        StatoPulsModSet = 1;
      if (diag == 1) Serial.println(" click SET/TEMP! "); 
        }        
      
      if (ModSetpoint == 1) {
        if (xpos > 60 && xpos < 118  && ypos > 200 && ypos < 230){    // PULSANTE UP
          StatoPulsIncr = 1;
          if (diag == 1) Serial.println(" click incremento Setpoint "); 
        }  
      
        if (xpos > 122 && xpos < 180  && ypos > 200 && ypos < 230){    // PULSANTE DOWN
          StatoPulsDecr = 1;
          if (diag == 1) Serial.println(" click decremento Setpoint ");
        }          
      }

      if (Alarm == 1) {
        if (diag == 1) Serial.println(" aspetto click Snooze "); 
        
        if (xpos > 60 && xpos < 180  && ypos > 90 && ypos < 190){    // PULSANTE SNOOZE
          StatoPulsAlarmOF = AlarmOF = 1;
          start_snooze = millis();
          if (diag == 1) Serial.println(" click Snooze "); 
        }
